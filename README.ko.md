...
6. Issue, diff, test, review 근거로 PR 메시지를 작성하고 기본적으로 GitHub PR까지 연다.
...
5. `pr-in-the-loop:open-pr` - 사람이 리뷰 상태를 승인한 뒤 근거 기반 PR 메시지를 작성하고, 명시적 opt-out이 없으면 기본적으로 GitHub PR까지 연다.
...
| `pr-in-the-loop:open-pr` | 근거 기반 PR 메시지를 작성하고 기본적으로 GitHub PR까지 열 때. |
