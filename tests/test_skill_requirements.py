...
class SkillRequirementTests(unittest.TestCase):
    ...
    def test_issue_skill_is_issue_only_and_requires_planning_pr_approval(self) -> None:
        ...
        self.assertNotIn("approved PR plan artifact and none exists", text)
    ...
    def test_planning_pr_creates_lightweight_pr_plan_from_issue(self) -> None:
        text = read_skill("planning-pr")
        agent = (SKILLS / "planning-pr" / "agents" / "openai.yaml").read_text(encoding="utf-8")
        dev_workflow = read_skill("github-dev-workflow")
        readme = read_root_file("README.md")
        readme_ko = read_root_file("README.ko.md")

        self.assertIn("Create a lightweight PR plan from an approved GitHub Issue", text)
        self.assertIn("REQUIRED SUB-SKILL:** Use `superpowers:brainstorming`", text)
        self.assertIn("A GitHub Issue exists or an issue draft has been explicitly accepted", text)
        self.assertIn("The human has explicitly approved moving from `pr-in-the-loop:issue` to `pr-in-the-loop:planning-pr`", text)
        self.assertIn("Locate the GitHub Issue or accepted issue draft", text)
        self.assertIn("Read the Issue or accepted draft", text)
        self.assertIn("Ambiguity Gate", text)
        self.assertIn("Do not write the PR plan while blocking ambiguity remains", text)
        self.assertIn("wait for human approval or selection", text)
        self.assertIn("propose 2-3 solution approaches with tradeoffs", text)
        self.assertIn("docs/pr-plans/", text)
        self.assertIn("issue-[issue number]-[issue title]-pr-[pr number].md", text)
        self.assertIn("If one Issue needs multiple PRs, write the PR plan for the first PR only", text)
        self.assertIn("Do not create a concrete implementation spec", text)
        self.assertIn("`pr-in-the-loop:parallel-development` owns the uncommitted concrete spec after this plan is approved", text)
        self.assertIn("Redaction And Plan Write Safety Gate", text)
        self.assertIn("Redact or summarize secrets, credentials, tokens, personal data, private URLs, and absolute local paths", text)
        self.assertIn("Before creating a new PR plan, search `docs/pr-plans/` for an existing plan with the same Issue number and PR number", text)
        self.assertIn("After creation, treat the PR plan path as the idempotency key", text)
        self.assertIn("Create lightweight PR plans from approved Issues", agent)
        self.assertIn("Issue-defined task into a human-reviewed PR plan", dev_workflow)
        self.assertIn("generate the uncommitted concrete spec through `pr-in-the-loop:parallel-development`", dev_workflow)
        self.assertIn("write a concise human-reviewed PR plan", readme)
        self.assertIn("짧은 PR 계획 문서", readme_ko)

    def test_planning_pr_template_uses_compact_required_sections(self) -> None:
        text = read_reference("planning-pr", "pr-plan-template.md")
        old_template = SKILLS / "planning-pr" / "references" / "spec-template.md"

        self.assertFalse(old_template.exists())
        self.assertIn("# [document title]", text)
        self.assertIn("For agentic workers: REQUIRED SUB-SKILL: Use `pr-in-the-loop:parallel-development`.", text)
        self.assertIn("This PR plan is not implementation permission", text)
        self.assertIn("Translate the headings and helper text to the detected user language", text)
        self.assertIn("## 목표", text)
        self.assertIn("## 이번 단계에 포함하는 것", text)
        self.assertIn("## 이번 단계에 포함하지 않는 것", text)
        self.assertIn("## 테스트 목표", text)
        self.assertIn("db schema", text)
    ...
    def test_parallel_development_generates_uncommitted_spec_from_pr_plan(self) -> None:
        text = read_skill("parallel-development")
        agent = (SKILLS / "parallel-development" / "agents" / "openai.yaml").read_text(encoding="utf-8")
        dev_workflow = read_skill("github-dev-workflow")
        planning = read_skill("planning-pr")
        readme = read_root_file("README.md")
        readme_ko = read_root_file("README.ko.md")
        gitignore = read_root_file(".gitignore")

        self.assertIn("Generate a concrete implementation spec from a human-approved PR plan", text)
        self.assertIn("REQUIRED SUB-SKILL:** Use `superpowers:writing-plans` discipline only", text)
        self.assertIn("A human-approved PR plan markdown file exists", text)
        self.assertIn("Save the generated spec under `docs/specs/` by default", text)
        self.assertIn("> For agentic workers: Never commit this file.", text)
        self.assertIn("Verify the generated spec is not staged, tracked, or committed before implementation and again before completion", text)
        self.assertIn("do not stop for the writing-plans execution-choice handoff", text)
        self.assertIn("Redaction And Spec Write Safety Gate", text)
        self.assertIn("Before creating a new spec, search `docs/specs/` for an existing spec with the same Issue number and PR plan path", text)
        self.assertIn("After the spec exists, continue with the existing file-level delegation flow", text)
        self.assertIn("Generate specs from PR plans, then run safe subagents", agent)
        self.assertIn("generate the uncommitted concrete spec through `pr-in-the-loop:parallel-development`", dev_workflow)
        self.assertNotIn("requires a concrete implementation spec and none exists", dev_workflow)
        self.assertNotIn("requires a concrete implementation spec and none exists", planning)
        self.assertIn("docs/specs/", gitignore)
        self.assertIn("generate an uncommitted implementation spec", readme)
        self.assertIn("커밋하지 않는 구현 spec을 생성", readme_ko)

    def test_parallel_development_spec_template_requires_never_commit_notice(self) -> None:
        template = read_reference("parallel-development", "spec-template.md")

        self.assertIn("# [PR name] Implementation Spec", template)
        self.assertIn("> For agentic workers: Never commit this file.", template)
        self.assertIn("**Issue:**", template)
        self.assertIn("**PR Plan:**", template)
        self.assertIn("## Scope", template)
        self.assertIn("## File Structure", template)
        self.assertIn("## Tasks", template)
        self.assertIn("## Self Review", template)
        self.assertIn("must not be staged, tracked, or committed", template)
    ...
    def test_multi_view_code_review_requires_parallel_spawn_and_model_effort_policy(self) -> None:
        ...
        self.assertIn("`gpt-5.4-mini` or `gpt-5.3-codex` + `medium`", text)
        self.assertIn("spec compliance and general code quality review", text)
        self.assertIn("`gpt-5.5` + `medium` or `high`", text)
